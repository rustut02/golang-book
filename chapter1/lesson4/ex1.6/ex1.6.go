package main

import (
	"image"
	"image/color"
	"image/gif"
	"io"
	"math"
	"math/rand"
	"os"
	"time"
)

var colorGreen = color.RGBA{G: 222, A: 255}
var colorRed = color.RGBA{R: 222, A: 255}
var colorBlue = color.RGBA{B: 222, A: 255}

var palette = []color.Color{color.Black, colorGreen, colorRed, colorBlue}

const (
	blackIndex = 0
	greenIndex = 1
	redIndex   = 2
	blueIndex  = 3
)

func SetColorIndex(frameNum int) uint8 {
	colorIndex := uint8(blackIndex)

	switch {
	case frameNum <= 2:
		colorIndex = greenIndex
	case frameNum >= 3 && frameNum <= 5:
		colorIndex = redIndex
	case frameNum >= 6:
		colorIndex = blueIndex
	}

	return colorIndex
}

func main() {
	lissajous(os.Stdout)
}

func lissajous(out io.Writer) {
	const (
		cycles  = 5
		res     = 0.001
		size    = 100
		nframes = 64
		delay   = 8
	)
	rand.Seed(time.Now().UTC().UnixNano())
	freq := rand.Float64() * 3.0
	anim := gif.GIF{LoopCount: nframes}
	phase := 0.0
	for i := 0; i < nframes; i++ {
		rect := image.Rect(0, 0, 2*size+1, 2*size+1)
		img := image.NewPaletted(rect, palette)
		for t := 0.0; t < cycles*2*math.Pi; t += res {
			x := math.Sin(t)
			y := math.Sin(t*freq + phase)
			img.SetColorIndex(size+int(x*size+0.5), size+int(y*size+0.5), SetColorIndex(i))
		}
		phase += 0.1
		anim.Delay = append(anim.Delay, delay)
		anim.Image = append(anim.Image, img)
	}
	gif.EncodeAll(out, &anim)

}
